  ...
  if (!cat) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        ...
        <Link to="/cats" className="label text-rose hover:underline cursor-pointer">← Back</Link>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen pt-24 pb-24 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, x: -14 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
          <Link to="/cats" className="inline-flex items-center gap-2 label text-ink-muted hover:text-ink transition-colors cursor-pointer mb-10">
            <ArrowLeft size={13} /> All cats · 所有猫咪
          </Link>
        </motion.div>
          ...
          >
            {cat.photo ? (
              ...
              >
                ...
                <span className="label opacity-40">Photo coming</span>
              </div>
            )}
          </motion.div>
        ...
        {next && (
          ...
          >
            ...
            <Link to={`/cats/${next.id}`} className="inline-flex items-center gap-3 group cursor-pointer">
              ...
              <ArrowRight size={15} className="text-ink-faint group-hover:text-rose transition-colors" />
            </Link>
          </motion.div>
        )}
      </div>
